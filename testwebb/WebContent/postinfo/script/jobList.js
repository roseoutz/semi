...
app.SearchForm = JK5Cript.View.extend({
    ...
    events: {
        ...
        'click #lb_imtFullTime': 'onFullTimeSelect',     // 정규직 여부
        ...
        // 관심기업
        'click .tplBtnFav': 'onMultiFavorSelect',  // 멀티
        'click .dev-favor, .dev-btn-favor': 'onSingleFavorSelect', // 싱글
        'click .dev-favorplus': 'onSingleFavorSelectPlus',
    ...
    _listOrderChange: function () {
        var that = this;
        var targetDuty = [1000067, 1000140, 1000088, 1000092, 1000089, 1000091, 1000093, 1000112, 1000113];
    ...
    _toolItemReset: function () {
        ...
        $(this.idWord + 'lb_imtFullTime').prop('checked', false).siblings('label').removeClass('on');
    ...
    onCareerItemSelect: function (e) {
        ...
        this._bindSingleItem({
            selector: e.currentTarget, limit: 5, category: 'career', alert_limit: '경력은'
        });
    },
    ...
    onEduItemSelect: function (e) {
        this._bindSingleItem({
            selector: e.currentTarget, limit: 6, category: 'edu', alert_limit: '학력은'
        });
    },
    onCoTypeItemSelect: function (e) {
        this._bindSingleItem({
            selector: e.currentTarget, limit: 10, category: 'cotype', alert_limit: '기업형태는'
        });
    },
    onJobTypeItemSelect: function (e) {
        this._bindSingleItem({
            selector: e.currentTarget, limit: 5, category: 'jobtype', alert_limit: '고용형태는'
        });
    },
    onPosition1ItemSelect: function (e) {
        this._bindSingleItem({
            selector: e.currentTarget, limit: 5, category: 'position1', alert_limit: '직급은'
        });
    },
    ...
    onPosition2ItemSelect: function (e) {
        this._bindSingleItem({
            selector: e.currentTarget, limit: 4, category: 'position2', alert_limit: '직책은'
        });
    },
    onPayItemSelect: function (e) {
        ...
        this._bindSingleItem({
            selector: e.currentTarget, limit: 6, category: 'pay', alert_limit: '연봉은'
        });
    },
    ...
    onLicenseSelect: function (e) {
        this._bindSingleItem({
            selector: e.currentTarget, limit: 10, category: 'license', alert_limit: '자격증은'
        });
    },
    onPrefItemSelect: function (e) {
        this._bindSingleItem({
            selector: e.currentTarget, limit: 10, category: 'pref', alert_limit: '우대조건은'
        });
    },
    onWelItemSelect: function (e) {
        this._bindSingleItem({
            selector: e.currentTarget, limit: 10, category: 'wel', alert_limit: '복리후생은'
        });
    },
            ...
            JK5Cript.Loader.async(options, function (response, succstat) {
                ...
                if (succstat) {
                    ...
                    if ($ul.find('li').length === 0) {
                        $ul.append(that.templete_item.format(category, that._makeKey(category, value, 3), value, highName, '전체', ''));
                    }
                    // 정보 입력
    ...
    onRecentSearchedTermsToggle: function (e) {
        ...
        this._toggleRecentSearchedLayer();
    },
    ...
    _getGIList: function (obj) {
        ...
        if (!that.isBusy) {
            var opts = {
                ...
                fulltime: $('#lb_imtFullTime').is(':checked') ? 1 : 0,
    ...
    onCllickLog: function (e) {
        ...
        var gno = (infos[0]);
        $.ajax({
            url: domain + '/Recruit/Home/_SmartMatchSummary?gno=' + gno,
            type: "GET",
            dataType: "html",
    ...
    onClickApplyEtc: function (e) {
        ...
        that._blankLocationHref(dataArray[0]);
    },
    ...
    onMultiFavorSelect: function (e) {
        ...
        $.each($('input[name=gno]:checked'), function (i, e) {
            ...
            arrCId.push(that.agiBaseInfo.memId);
        });
        // 중복 제거
        arrCId = arrCId.reduce(function (a, b) {
            if (a.indexOf(b) < 0) a.push(b);
            return a;
        }, []);
        that._favorSelect(arrCId.join(','), 1);
    },
    onSingleFavorSelectPlus: function (e) {
        var $selector = $(e.currentTarget);
        var onClass = 'on';
        var $target = $selector;
        var memId;
        if ($selector.hasClass('dev-btn-favor')) {
            $target = $selector.closest('span');
        }
        if (_.isUndefined($selector.attr('data-value'))) {
            this.agiBaseInfo.setBaseInfo($selector.closest('li').attr('data-info'));
            memId = this.agiBaseInfo.memId;
        } else {
            memId = $selector.attr('data-value');
        }
        var isOn = $target.hasClass(onClass) ? false : true;
        this._favorSelect(memId, isOn);
    },
    ...
    _favorSelect: function (arrCId, isOn) {
        var that = this;
        if (arrCId === '') {
            ...
            app.svcFunc.loginCheck(function (response) {
                var memCheck = response.memCheck;
                var fncId = that._makeFuncId();
                app.SearchForm.func[fncId] = app.SearchForm._wrapFunction(that._favor, that, [arrCId, isOn]);
                if (memCheck == 0) {
                    app.SearchForm.open('', fncId);
                } else if (memCheck == 1) {
                    app.SearchForm._callbackTest(fncId);
    ...
    _favor: function (arrCId, isOn) {
        var that = this;
        var options = {
            type: 'GET',
            url: '/List_GI/GI_FavorCo.asp',
            data: 'rCID=' + arrCId + '&rFavorStat=' + (isOn ? 1 : 2),
            contentType: 'charset=utf-8',
        };
        JK5Cript.Loader.async(options, function (response, succstat) {
            if (succstat) {
                if (response == '1' || response == '4') {
                    var alertMsg = isOn ? '관심기업 설정이 완료 되었습니다.' : '관심기업 해제가 완료 되었습니다.';
                    alert(alertMsg);
                    that._setChangeUI('tplBtnFav', arrCId, isOn);
                } else if (response == '3') {
                    alert("이미 관심기업을 100개 다 설정하셨습니다.\n\n관심기업 목록에서 다른 기업을 삭제하신 후, 다시 설정해 주시기 바랍니다");
                } else {
                    if (response !== undefined && response.indexOf('html') > -1) {
                        that._insertToFrame(response);
                    }
                }
            }
        });
    ...
    onFullTimeSelect: function (e) {
        ...
        this._getGIList({ page: 1, fulltime: isChecked ? 1 : 0 });
        if (isChecked) {
